...
export class TransactionListQuery {
  ...
  isValid(): { valid: boolean; errors: string[] } {
    ...
    if (this.minAmount !== undefined && (!Number.isFinite(this.minAmount) || this.minAmount < 0)) {
      errors.push('Minimum amount must be a non-negative number');
    }

    if (this.maxAmount !== undefined && (!Number.isFinite(this.maxAmount) || this.maxAmount < 0)) {
