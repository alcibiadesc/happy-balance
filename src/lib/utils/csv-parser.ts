...
export function parseCSV(content: string): ParseResult {
  ...
  if (lines.length < 2) {
    return {
      transactions: [],
      errors: [
        { row: 0, message: "CSV must have header and at least one data row" },
      ],
      headers: [],
    };
  }
    ...
    try {
      ...
      if (transaction) {
        ...
        if (existingHash) {
          ...
          transaction.duplicateReason = getTranslation("data.transaction_exists");
        } else {
      ...
      errors.push({
        row: i + 1,
        message:
          error instanceof Error ? error.message : getTranslation("data.parsing_error"),
        data: lines[i],
      });
    }
  }
...
function mapToTransaction(
  ...
  try {
    ...
    if (!partner || partner.replace(/[^\w\s]/g, "").trim() === "") {
      partner = getTranslation("data.unknown_merchant");
    }
