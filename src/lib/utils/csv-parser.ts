...
function createTransactionHash(date: string, partner: string, amount: number): string {
...
function detectFieldMapping(headers: string[]): Record<string, number> {
  ...
  const fieldPatterns = {
    ...
    paymentReference: /payment\s*reference|referencia\s*pago|concepto|description|descripci[oó]n/i,
...
function mapToTransaction(values: string[], mapping: Record<string, number>, rowNumber: number): ParsedTransaction | null {
  try {
    ...
    const hash = createTransactionHash(dateStr, partner, amount);
