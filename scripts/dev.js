...
function getWorkspaceInfo() {
  try {
    ...
    const workspaceName = isWorktree ? process.cwd().split("/").pop() : "main";
...
function startServers(ports) {
  ...
  const frontend = spawn(
    "pnpm",
    ["vite", "dev", "--host", "0.0.0.0", "--port", ports.frontend.toString()],
    {
      cwd: rootDir,
      stdio: "pipe",
      shell: true,
...
async function setup() {
  ...
  log(
    `\n📁 Workspace: ${workspaceInfo.name} (${workspaceInfo.branch})`,
    "blue",
  );
  log(
    `📍 Type: ${workspaceInfo.isWorktree ? "Worktree" : "Main Repository"}`,
    "blue",
  );
...
async function dev() {
  ...
  log(
    `  Database: postgresql://localhost:${DB_CONFIG.port}/${dbName}`,
    "yellow",
  );
