  ...
  describe('execute', () => {
    ...
    it('should calculate trends for different periods', async () => {
      ...
      for (const period of periods) {
        ...
        mockFinancialCalculationService.calculateCategoryBreakdown.mockReturnValue(Result.ok([]));
        mockFinancialCalculationService.calculateSpendingRate.mockReturnValue(Result.ok(0));
        mockFinancialCalculationService.calculateExpenseDistribution.mockReturnValue(
          Result.ok({
        ...
        mockFinancialCalculationService.calculateTrends.mockReturnValue(Result.ok([]));
        ...
        // Assert
        expect(result.isSuccess()).toBe(true);
        expect(mockFinancialCalculationService.calculateTrends).toHaveBeenCalled();
      }
    });
  });
});
