...
export function createBackupRoutes(): Router {
  ...
  // Get backup policy (must be before /:id to avoid being caught by param route)
  router.get(
    '/policy/current',
    asyncHandler(async (req: Request, res: Response) => {
      try {
  ...
  // Update backup policy (must be before /:id to avoid being caught by param route)
  router.put(
    '/policy',
    asyncHandler(async (req: Request, res: Response) => {
      try {
        const userId = req.user?.userId || 'default';
        const { enabled, frequency, hourUtc, dayOfWeek, dayOfMonth, retentionDays, maxBackups } =
          req.body;
  ...
  // Cleanup expired backups (manual trigger, must be before /:id)
  router.post(
    '/cleanup',
    asyncHandler(async (req: Request, res: Response) => {
      try {
