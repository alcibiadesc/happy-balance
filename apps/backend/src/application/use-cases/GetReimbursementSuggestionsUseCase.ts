...
export class GetReimbursementSuggestionsUseCase {
  ...
  async execute(
    ...
    const candidates = incomeResult
      .getValue()
      .transactions.filter((tx) => !tx.linkedTransactionId && !tx.isReimbursement)
    ...
    for (const income of candidates) {
      ...
      if (best.match.matchScore < minScore) continue;
      ...
      suggestions.push({
        ...
        matchReasons: best.match.matchReasons,
