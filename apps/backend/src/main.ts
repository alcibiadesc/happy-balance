...
class App {
  ...
  private initializeRoutes() {
    ...
    this.app.get('/version', (req, res) => {
      try {
        ...
        if (fs.existsSync(versionFile)) {
          ...
          // Fallback for local development - read package.json
    ...
    this.app.use('/api/admin/users', createUserManagementRoutes(this.userManagementController));

    // API routes (auth required for most)
    // Use new V2 routes with controller factory
    this.app.use('/api/transactions', createTransactionRoutesV2(this.controllerFactory));
    this.app.use('/api/categories', createCategoryRoutesV2(this.controllerFactory));
    this.app.use('/api/dashboard', createDashboardRoutesV2(this.controllerFactory));
    this.app.use('/api/import', createImportRoutesV2(this.controllerFactory));
    this.app.use('/api/metrics', createMetricsRoutesV2(this.controllerFactory));

    // V2 routes using factory pattern for all functionality
    this.app.use('/api/preferences', createUserPreferencesRoutesV2(this.controllerFactory));
    this.app.use('/api/seed', createSeedRoutesV2(this.controllerFactory));

    // Widget Settings routes
    this.app.use('/api/widgets', createWidgetSettingsRoutesV2(this.controllerFactory));

    // Investment/Portfolio routes
    this.app.use('/api/investments', createInvestmentRoutesV2(this.controllerFactory));

    // Export routes
    this.app.use('/api/export', createExportRoutes(this.controllerFactory));

    // Backup routes
    this.app.use('/api/backups', createBackupRoutes());

    // Merchant Alias routes (intelligent tagging)
    this.app.use('/api/merchant-aliases', createMerchantAliasRoutesV2(this.controllerFactory));
