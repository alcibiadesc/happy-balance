...
export class Category {
  ...
  static create(
    ...
    return Result.ok(new Category(categoryId, name.trim(), color.toLowerCase(), icon.trim(), type));
  }
  ...
  static fromSnapshot(snapshot: CategorySnapshot): Result<Category> {
    ...
    if (!typeResult) {
      return Result.failWithMessage(`Invalid transaction type: ${snapshot.type}`);
    }
